...
async function getDb() {
  ...
  const initializeFreshDb = () => {
    console.log("\u{1F504} Initializing fresh embedded SQLite database with schema and seed data...");
  ...
  if (import_fs.default.existsSync(DB_FILE)) {
    try {
      ...
      try {
        ...
        import_fs.default.renameSync(DB_FILE, backupPath);
        console.log(`\u{1F4E6} Quarantined corrupted DB file to: ${backupPath}`);
      } catch (_) {
        try {
  ...
  if (process.env.NODE_ENV === "production" || hasDist) {
    ...
    try {
      const vite = await (0, import_vite.createServer)({
        server: { middlewareMode: true, hmr: { server: void 0 } },
        appType: "spa"
      });
